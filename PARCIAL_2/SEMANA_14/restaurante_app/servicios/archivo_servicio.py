...
class ArchivoServicio:
    ...
    def cargar_productos(self) -> List[Dict[str, Any]]:
        try:
            with open(self.productos_path, "r", encoding="utf-8") as archivo:
    ...
    def guardar_productos(self, productos: List[Dict[str, Any]]) -> None:
        with open(self.productos_path, "w", encoding="utf-8") as archivo:
            json.dump(productos, archivo, ensure_ascii=False, indent=2)

    def cargar_usuarios(self) -> List[Dict[str, Any]]:
        try:
            with open(self.usuarios_path, "r", encoding="utf-8") as archivo:
                data = json.load(archivo)
            if not isinstance(data, list):
                raise json.JSONDecodeError("Se esperaba una lista", doc=str(data), pos=0)
            return data
    ...
    def guardar_usuarios(self, usuarios: List[Dict[str, Any]]) -> None:
        with open(self.usuarios_path, "w", encoding="utf-8") as archivo:
